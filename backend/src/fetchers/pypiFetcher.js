// /backend/src/fetchers/pypiFetcher.js
// Purpose: The core logic for proxying and caching PyPI packages.
...
async function handlePypiRequest(req, res, next) {
    try {
        ...
        // Check if this is a package file request (.whl or .tar.gz)
        ...
        // Parse package name and version from the URL path
...
async function fetchAndCachePypiPackage(packageName, version, filename, urlPath) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Parse the HTML to find the link for our specific filename
